...
class UtilTests(unittest.TestCase):
    ...
    def test_draw(self):
        ...
        Phylo.draw(apaf, do_show=False, save_name="test")
        assert os.path.exists("test.png")
        if not os.stat("test.png").st_size > 1:
            raise FileNotFoundError(
                "Size of the saved graph is too small.") from None
        os.remove("test.png")
    ...
    def test_draw_with_label_colors_callable(self):
        ...
        def label_colors_dollo(label): return "r" if label == "f_50" else "k"
        def label_colors_apaf(label): return "r"  # noqa: E731
    ...
    def test_visualize_changes(self):
        ...
        if not os.stat("test_gif1.gif").st_size > 1:
            raise FileNotFoundError(
                "Size of the saved gif is too small.") from None
        if not os.stat("test_gif2.gif").st_size > 1:
            raise FileNotFoundError(
                "Size of the saved optimized gif is too small.") from None
